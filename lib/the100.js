...
function notify(groupConfig, game, uuid) {
    ...
    if (config.notify && moment(game.time).isAfter(moment())) {
        ...
        } else if (!moment(game.time).isBefore(moment())) {
        ...
        request.put({
            ...
            log.info(util.format("Set notification status to '%'", body.entities[0].notification));
        });
    }
}
...
function processScrape(groupConfig, game) {
    ...
    request.get({
        ...
        } else if (ok(body).has('body.entities.0.notification') && 
            body.entities[0].notification !== "delivered" &&
            body.entities[0].notification !== "ignored") {
            if (parseInt(game.groupId) === parseInt(groupConfig.id)) {
